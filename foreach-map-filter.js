...
function showFirstAndLast(arr){
    const firstLastChars = [];
    arr.forEach(word => {
        const firstLetter = word[0];
        const lastLetter = word[word.length - 1];
        firstLastChars.push(firstLetter + lastLetter);
    })
...
function vowelCount(str){
    const lowercaseArr = str.toLowerCase().split(""); // make function case insensitive
    const vowelObj = {};
...
function removeVowels(str) {
    ...
    return lowerCaseStrArr.filter(char => 'aeiou'.indexOf(char) === -1).join('');
}
